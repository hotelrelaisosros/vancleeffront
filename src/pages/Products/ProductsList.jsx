...
const ProductsList = () => {
    ...
    useEffect(() => {
        ...
        if (parseInt(categoryID) === 2) {
            navigate('/products/ring-two');
        }
    }, [categoryID])
    ...
    useEffect(() => {
        const formData = { sub_category_id: categoryID };
        dispatch(ProductsMiddleware.GetAllNonRingProducts(formData));
    }, [dispatch]);
                    ...
                    <div className="products-card-main">
                        <div className="row">
                            {filteredProducts?.map((product, index) => (
                                <div key={index} className='product-box col-sm-12 col-md-6 col-lg-4 col-xl-4' onClick={() => handleClickOnProduct(product)}>
                                    { (product?.images.length) ? <img src={product?.images[0]?.image} alt={product.title}  /> : <img src="../img/MOQEqajYdECDwcsmouTOdA.jpeg.transform.vca-w350-1x.avif" /> }
                                    <h5>{product.title}</h5>
                                    <p>{product.desc} </p>
                                    <p>{product.price} €</p>
                                </div>
